...
def _fila(url, tipo, valor, miembros, horas=48, enviado=True, idq=None):
    return {"cuando_utc": (AHORA - timedelta(hours=horas)).isoformat(), "enviado": enviado,
            "url": url, "id_pregunta": idq, "tipo": tipo, "pregunta": f"¿{url}?",
            "valor": valor, "miembros": miembros, "torneo": 33121}
...
def test_puntos_binaria_opciones_y_numerica():
    ...
    assert mc.puntos("multiple_choice", {"A": 0.6, "B": 0.4}, "B")["log"] == pytest.approx(math.log(0.4))
...
def test_pregunta_de_un_grupo_por_su_id():
    post = {"group_of_questions": {"questions": [{"id": 5, "status": "open"}, {"id": 6, "status": "resolved"}]}}
...
def test_resumen_lee_la_puntuacion_oficial():
    q = {"status": "resolved", "resolution": "yes",
         "my_forecasts": {"score_data": {"spot_peer_score": 12.5, "peer_score": 10.0}}}
    assert mc.resumen_pregunta(q) == {"estado": "resolved", "resolucion": "yes",
                                      "spot_peer": 12.5, "peer": 10.0, "baseline": None}
...
def test_calcular_por_modelo_calibracion_y_peores():
    ...
    resueltas = {str(mc.clave(filas[0])): {"estado": "resolved", "resolucion": "yes", "spot_peer": 20.0},
...
def test_programa_completo_con_metaculus_simulado(tmp_path, monkeypatch):
    ...
    fila = _fila("https://www.metaculus.com/questions/777/", "binary", 0.7,
                 [{"modelo": "a", "valor": 0.7}], horas=24 * 30)
    ...
    pedidas = []
    monkeypatch.setattr(mc, "pedir_post", lambda pid, token: pedidas.append(pid) or {
        "question": {"id": 1, "status": "resolved", "resolution": "yes",
    ...
    assert pedidas == [777]
    assert json.loads((tmp_path / "marcador.json").read_text(encoding="utf-8"))["spot_peer_suma"] == 8.0
