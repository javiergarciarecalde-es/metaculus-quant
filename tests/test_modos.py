...
from tests.conftest import (RESPUESTA_BINARIA, MetaculusFalso, ModeloFalso,
                            preguntas_ejemplo)
...
def _bot_con_puestos(llms, puestos, respaldos):
    params = cfg.cargar_params()
    bot = main.construir_bot(params, publicar=False,
                             llms={**llms, "_puestos": puestos, "_respaldos": respaldos})
...
def test_si_fallan_dos_de_tres_puestos_no_se_envia(llms):
    bueno = ModeloFalso()
    bot = _bot_con_puestos(llms, [ModeloRoto("excepcion"), ModeloRoto("vacio"), bueno],
                           [None, None, None])
...
JSON_OK = '[{"dato": "fuente de resolución", "busqueda": "q1"}, {"dato": "tasa base", "busqueda": "q2"}]'


def _ampliar(director, buscador, tope=5):
    return asyncio.run(inv.ampliar("INFORME BASE", "¿X?", "criterios", director, buscador,
                                   n=2, tope_segundos=tope))
